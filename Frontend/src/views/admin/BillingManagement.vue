        ...
        <el-tab-pane :label="t('Subscriptions & Credits', 'Subscriptions & Credits')" name="subscriptions">
          <section class="admin-surface" v-loading="loadingUsers">
            ...
            <el-table :data="filteredUsers" class="billing-table" table-layout="auto">
              ...
              <el-table-column :label="t('Credits', 'Credits')" min-width="260"><template #default="{ row }"><div class="credit-cell"><div><strong>{{ row.totalRemainingCredits ?? row.remainingCredits }}</strong><span>{{ t('ví còn lại', 'wallet remaining') }}</span></div><small>{{ row.creditBuckets?.length || 0 }} {{ t('bucket', 'buckets') }} · {{ row.usedCredits }} {{ t('đã dùng', 'used') }}</small><div v-for="bucket in row.creditBuckets || []" :key="bucket.id" class="credit-bucket-detail"><span>{{ String(bucket.sourcePlan).toUpperCase() }}: {{ bucket.remaining }} {{ t('còn lại', 'remaining') }}</span><em>{{ bucket.status }} · {{ formatDate(bucket.expiresAt) }}</em></div></div></template></el-table-column>
...
.credit-bucket-detail { display: flex; justify-content: space-between; gap: 8px; font-size: 11px; }.credit-bucket-detail em { color: var(--color-text-muted); font-style: normal; }
</style>
