...
test('public navigation exposes the right auth CTAs without a self-login link', () => {
  ...
  assert.match(mobileNav, /<router-link[^>]+to="\/register"/)
  assert.match(home, /:aria-expanded="mobileOpen"/)
  assert.match(home, /aria-controls="landing-mobile-nav"/)
  assert.match(mobileNav, /class="btn btn-secondary mobile-login-cta"/)
  assert.match(mobileNav, /class="btn btn-primary mobile-register-cta"/)
  assert.ok(mobileNav.indexOf('to="/login"') < mobileNav.indexOf('to="/register"'))
  ...
  assert.match(home, /\.mobile-cta-stack \.btn,[\s\S]*?width:100%;[\s\S]*?min-height:44px !important;/)
  assert.match(home, /\.mobile-cta-stack \.btn-secondary \{[\s\S]*?border:1px solid var\(--line-strong\) !important;[\s\S]*?background:transparent !important;/)
  assert.match(home, /\.mobile-cta-stack \.btn-primary \{[\s\S]*?background:linear-gradient[\s\S]*?!important;/)
})
